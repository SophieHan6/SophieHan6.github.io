...
var teslaThemes = {
  ...
  fullPageInit: function () {
    if (jQuery("#fullpage").length) {
      jQuery("#fullpage").fullpage({
        ...
        menu: "#fp-nav", //绑定菜单，设定的相关属性与anchors的值对应后，菜单可以控制滚动，默认为false。
        controlArrows: false, //定义是否通过箭头来控制slide,默认true
        verticalCentered: false, //定义每一页的内容是否垂直居中，默认true
        easing: "swing", //定义页面section滚动的动画方式，若修改此项需引入jquery.easing插件
        normalScrollElements: ".single-project-slide, .blogpost-slide", //避免自动滚动，滚动时的一些元素，例如百度地图
        afterLoad: function (anchorLink, index) {
          // console.log("触发", anchorLink, index);
          // $(`section[data-anchor='${section}']`).slideDown()
          teslaThemes.logoFix();
        },
  ...
  contactFormToggles: function () {
    jQuery(
      ".contact-form .input-group input, .contact-form .input-group textarea, .search-form input"
    ).focusout(function () {
  ...
  projectCoverSlider: function () {
    if (jQuery(".project-cover").length) {
      jQuery(".project-cover ul").owlCarousel({
        ...
        rewindNav: false,
        afterInit: function () {
          jQuery(".project-cover .slider-nav span").text(
            this.owl.currentItem + 1 + " from " + this.owl.owlItems.length
          );
        },
  ...
  horizontalNavigation: function () {
    jQuery(".portfolio-item .details, .blog-item .view-post").on(
      "click",
      function (e) {
        e.preventDefault();
  ...
  logoFix: function () {
    if (
      jQuery("body").hasClass("fp-viewing-portfolio") ||
      jQuery("body").hasClass("fp-viewing-blog")
    ) {
